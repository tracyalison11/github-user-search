<template>
  <div>
    ...
    <p class="search-results-info error" v-if="error && !loaded">An error occurred during search. You may have exceeded the rate limit. Please wait and try again.</p>
    <p class="search-results-info" v-if="loaded">Total search results: {{Number(this.totalResults).toLocaleString()}}</p>

    <div class="empty-screen" v-if="!loaded">
      <img src="../assets/inspectocat.jpg" class="inspectocat" alt="inspector character"/>
      <p class="directions">Mouse over the search icon to look for friends!</p>
    </div>
